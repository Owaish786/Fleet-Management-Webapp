...
import { ArrowRight, Radar, Route, ShieldCheck, Truck, Users, Wrench } from 'lucide-react'
...
export function LandingPage() {
  ...
  const primaryCta = user ? { to: '/dashboard', label: 'Open dashboard' } : { to: '/signup', label: 'Get started' }
  const secondaryCta = user ? { to: '/login', label: loading ? 'Restoring session...' : 'Switch account' } : { to: '/login', label: 'Login to workspace' }
        ...
        <header className="flex items-center justify-between rounded-[1.6rem] border border-white/70 bg-white/70 px-4 py-3 shadow-sm shadow-slate-900/5 backdrop-blur-xl sm:px-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className="rounded-full bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-brand-600/20 transition-colors hover:bg-brand-700"
            >
        ...
        <main className="grid flex-1 gap-8 py-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:gap-10">
          <section className="flex flex-col justify-center">
            ...
            <h1 className="animate-fade-up mt-5 max-w-2xl font-display text-4xl font-black tracking-tight text-slate-950 sm:text-5xl lg:text-6xl">
              Run dispatch, tracking, and fleet readiness from one clean workspace.
            </h1>
            <p className="animate-fade-up delay-1 mt-5 max-w-xl text-lg leading-8 text-slate-600">
              Coordinate trips, keep drivers and vehicles aligned, and monitor live fleet movement without bouncing between disconnected tools.
            </p>

            <div className="animate-fade-up delay-2 mt-7 flex flex-wrap gap-3">
              <Link
                to={primaryCta.to}
                className="inline-flex items-center gap-2 rounded-2xl bg-brand-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-brand-600/20 transition-colors hover:bg-brand-700"
              >
                {primaryCta.label}
                <ArrowRight size={16} />
              </Link>
              <Link
                to={secondaryCta.to}
                className="rounded-2xl border border-white/80 bg-white/85 px-5 py-3 text-sm font-medium text-slate-600 shadow-sm shadow-slate-900/5 transition-colors hover:text-slate-900"
              >
                {secondaryCta.label}
              </Link>
            </div>
            ...
            <div className="animate-fade-up delay-3 mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {signalCards.map(({ label, value, icon: Icon }) => (
                <div key={label} className="rounded-[1.4rem] border border-white/80 bg-white/80 p-4 shadow-sm shadow-slate-900/5 backdrop-blur-xl">
                  ...
                  <p className="mt-2 text-sm text-slate-500">Built for daily transport operations.</p>
                </div>
              ))}
            </div>
          </section>
            ...
            <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
              <div className="rounded-[1.8rem] border border-slate-200 bg-[linear-gradient(145deg,rgba(79,70,229,0.08),rgba(14,165,233,0.04),rgba(255,255,255,0.9))] p-5">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mt-2 text-2xl font-bold tracking-tight text-slate-900">A sharper operating view</h2>
                  </div>
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-white/80 bg-white/85 p-4 shadow-sm shadow-slate-900/5">
                    ...
                    <p className="mt-3 text-3xl font-black tracking-tight text-slate-900">24</p>
                    <p className="mt-2 text-sm text-slate-500">Assets visible across readiness, maintenance, and dispatch.</p>
                  </div>
                  <div className="rounded-2xl border border-white/80 bg-white/85 p-4 shadow-sm shadow-slate-900/5">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Trip pressure</p>
                    <p className="mt-3 text-3xl font-black tracking-tight text-slate-900">8</p>
                    <p className="mt-2 text-sm text-slate-500">Routes in motion or scheduled inside one dispatch board.</p>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="rounded-[1.6rem] border border-slate-200 bg-slate-50/90 p-4">
                  <div className="flex items-center justify-between">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Core modules</p>
                    <Wrench size={16} className="text-slate-400" />
                  </div>
                  <div className="mt-4 space-y-3">
                    {['Vehicle registry', 'Driver roster', 'Dispatch board', 'Maintenance queue'].map((item) => (
                ...
                <div className="rounded-[1.6rem] border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/5">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Access flow</p>
                  <p className="mt-2 text-lg font-bold tracking-tight text-slate-900">Login or create an operator account</p>
                  <p className="mt-2 text-sm leading-6 text-slate-600">
                    Start from this landing page, then move into a secure workspace for day-to-day fleet operations.
                  </p>
                </div>
              </div>
            </div>
        ...
        <section className="pb-6">
          <div className="rounded-[2rem] border border-white/75 bg-white/75 p-5 shadow-sm shadow-slate-900/5 backdrop-blur-xl">
            ...
            <div className="mt-6 flex flex-col items-center justify-center rounded-[1.8rem] border border-slate-200 bg-[linear-gradient(180deg,rgba(255,255,255,0.9),rgba(241,245,249,0.9))] px-6 py-8 text-center">
              <h3 className="font-display text-3xl font-black tracking-tight text-slate-950">Start with a cleaner fleet workflow</h3>
              <p className="mt-3 max-w-2xl text-base leading-7 text-slate-600">
                Land here first, choose your access path, and move into a dashboard built for transport operations instead of generic admin screens.
              </p>
              <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
                <Link
                  to={user ? '/dashboard' : '/signup'}
                  className="rounded-2xl bg-emerald-500 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-emerald-500/20 transition-colors hover:bg-emerald-600"
                >
                  {user ? 'Open dashboard' : 'Create account'}
                </Link>
                <Link
                  to="/login"
                  className="rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition-colors hover:text-slate-900"
                >
